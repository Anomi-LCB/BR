        ...
        <div ref={feedRef} className="flex-1 overflow-y-auto px-3 py-3 space-y-2 no-scrollbar">
            {messages.length === 0 && (
                <div className="h-full flex flex-col items-center justify-center text-center opacity-40 space-y-3 pt-16">
                    ...
                    <p className="text-sm font-medium">아직 나눔이 없습니다.<br />오늘 받은 은혜를 가장 먼저 나눠보세요.</p>
                </div>
            )}
...
export default function GroupDashboardView({ user, streak, progress, onTabChange }: GroupDashboardViewProps) {
    const {
        groupId,
        members,
        messages,
    ...
    const [newGroupTheme, setNewGroupTheme] = useState("매일 1장 말씀 묵상");
    const THEMES = ["매일 1장 말씀 묵상", "성언 지킴이 친구", "바울서신 깊이 읽기", "모세오경 정복", "자유로운 나눔"];
    ...
    if (!effectiveUser) {
        if (!cachedAnon) {
            const newName = '익명의 성도' + Math.floor(Math.random() * 900 + 100);
            if (typeof window !== 'undefined') localStorage.setItem('anon_user', newName);
            effectiveUser = { id: 'anon_' + Date.now(), email: newName + '@anon.com', user_metadata: { first_name: newName } };
        } else {
            effectiveUser = { id: 'anon_cached', email: cachedAnon + '@anon.com', user_metadata: { first_name: cachedAnon } };
        }
    }

    const myName = effectiveUser?.user_metadata?.first_name || cachedAnon || '익명';

    // Initialize
    useEffect(() => {
        const saved = typeof window !== 'undefined' ? localStorage.getItem('bible_group_id') : null;
        if (saved && !groupId) {
    ...
    const handleSend = () => {
        if (inputText.trim()) {
            sendMessage(inputText, 'chat', effectiveUser);
            setInputText("");
        }
    };
            ...
            <div className="h-[80vh] flex flex-col items-center justify-center space-y-8 px-6 animate-in fade-in zoom-in-95 duration-700">
                <div className="text-center space-y-4">
                    ...
                    <p className="text-muted-foreground text-sm leading-relaxed max-w-[250px] mx-auto">
                        공동체와 함께 말씀을 묵상하고<br />받은 은혜를 나누어 보세요.
                    </p>
                </div>
                            ...
                            >
                                <div className="space-y-6 relative z-10">
                                    <div className="text-center space-y-2">
                                        <h3 className="text-xl font-serif font-black">코이노니아 신설</h3>
                                        <p className="text-xs text-muted-foreground font-medium">새로운 소그룹을 만들고 지체들을 초대하세요.</p>
                                    </div>

                                    <div className="space-y-4">
                                        <div className="space-y-2">
                                            <label className="text-[11px] font-bold text-muted-foreground uppercase tracking-widest pl-1">그룹 이름</label>
                                            <input
                                                type="text"
                                        ...
                                        <div className="space-y-2">
                                            <label className="text-[11px] font-bold text-muted-foreground uppercase tracking-widest pl-1">추천 테마 (선택)</label>
                                    ...
                                    <div className="pt-2">
                                        ...
                                        >
                                            {isJoining ? <Loader2 className="w-5 h-5 animate-spin" /> : '모임 시작하기 (초대 링크 생성)'}
                                        </button>
                                    </div>
                                </div>
                            </motion.div>
    ...
    return (
        <div className="fixed inset-0 z-40 flex flex-col bg-background">
            {/* Compact Chat Header */}
            <div className="shrink-0 bg-card/95 backdrop-blur-xl border-b border-border/50 shadow-sm pt-4">
                <div className="flex items-center justify-between px-4 py-2.5">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <div className="flex items-center gap-1.5">
                                ...
                                >
                                    {members.length}명 · {onlineCount} 접속
                                </button>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="flex items-center gap-1">
                        ...
                        <button
                            onClick={() => {
                                if (confirm("정말 그룹에서 나가시겠습니까?")) leaveSquad();
                            }}
                ...
                {showMembersPopup && (
                    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
                        ...
                        >
                            ...
                            <h3 className="text-sm font-black mb-3">참여 멤버 ({members.length}명)</h3>
                                    ...
                                    <div key={m.name || i} className="flex items-center gap-3 p-2 rounded-xl hover:bg-muted/40 transition-colors">
                                        ...
                                        <div className="flex-1 min-w-0">
                                            ...
                                            <p className="text-[10px] text-muted-foreground">
                                                {m.isOnline ? <span className="text-green-500 font-bold">🟢 접속 중</span> : '오프라인'}
                                            </p>
                                        </div>
